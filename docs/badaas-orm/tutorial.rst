...
for more details you can read :ref:`badaas-orm/query:Compilable query system`.
...
.. code-block:: go
    ...
    cities, err := crudCityService.Query(
        conditions.CityName(orm.Eq("Paris")),
        conditions.CityPopulation(orm.Gt(1000000)),
    )
...
.. code-block:: go
    ...
    cities, err := crudCityService.Query(
        ...
        conditions.CityCountry(
            conditions.CountryName(orm.Eq("France")),
        ),
    )
...
to which the service/repository used belongs (City in this case) but not of its relationships. 
If we also want to obtain this data, we must perform preloading.
...
As you can see, now the country attribute is a valid pointer to a Country object, with the corresponding information. 
Again, the Capital attribute of the Country is null, to obtain it we should do the corresponding preloading.

In this fourth tutorial we have used preloading, 
